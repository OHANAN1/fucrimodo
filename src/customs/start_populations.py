...
from src.fucrimodo.utils import artificial_crystal_creation as crystal_creation
...
class RandomSelectionPopulation(StartPopulation):
    """
...
class TournamentSelectionPopulation(StartPopulation):
...
class DiversityPopulation(StartPopulation):
    """
...
class BestCrystalsPopulation(StartPopulation):
    """
...
class WorstCrystalsPopulation(StartPopulation):
    """
    A class that returns a list of n crystals with the highest fitness.
    Set database_name to describe the database the original crystals are from.
...
class SelectAllPopulation(StartPopulation):
    """
...
class DopeStartPopulation(StartPopulation):
    ...
    def __repr__(self) -> str:
        return "DopeStartPopulation(add_n={}, atom_types={}, cell_bounds={})".format(
            self.add_n, self.atom_types, self.cell_bounds
        )

class ReplaceAndReaddLater(StartPopulation):
