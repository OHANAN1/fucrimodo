...
class CLICommand:
    ...
    def add_arguments(parser):
        ...
        add(
            '-s', '--save_dir',
            help=('Directory where the utilitie should save the results. '
                'If not provided, it is set to the current working directory.')
        )
        add(
            '-p', '--path',
            help=('Path to the file or directory the utilitie should act on. '
...
class Runner:
    ...
    def parse(self, args):
        ...
        assert self.utility_name in self.possible_utilities, \
            "The utility name is not valid. Possible utilities are: " + \
            ", ".join(self.possible_utilities)
    ...
    def __create_target_file(self):
        ...
        if self.path.endswith(".db"):
            ...
            database = db_tools.connect_to_existing_database(self.path)
        ...
        if self.config is None:
            from fucrimodo.lab_template.configs.utils.create_target_file_data import main
        else:
